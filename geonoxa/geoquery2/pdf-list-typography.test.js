...
test('PDF tailings typography stays legible and centrally configured', () => {
  ...
  assert.doesNotMatch(css, /\.geoquery-pdf-export[^{}]*tailings-list[^{}]*font-size:\s*[678](?:\.\d+)?px/);
});

test('PDF list gains space through geometry and keeps names on one line', () => {
  assert.match(css, /\.geoquery-pdf-export \.geoquery-list\{gap:2px\}/);
  assert.match(css, /\.geoquery-pdf-export \.geoquery-list__item\{[^}]*min-height:31px;[^}]*padding:3px 6px;[^}]*column-gap:6px/);
  assert.match(css, /\.geoquery-pdf-export \.geoquery-list__name\{font-size:10px;font-weight:400;line-height:11px\}/);
});
