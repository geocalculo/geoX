...
function drawResult(map,layers,group){ const cfg=group.cfg, res=group.result; if(res.status!=="resolved") return; const style=cfg.estilo||{}; if(cfg.id==="relaves"){ const rels=(res.selectedRelaves || res.items || []).slice(0,10); rels.forEach((r,index)=>{ const nearest=index===0; L.circleMarker([r.coordinates[1],r.coordinates[0]],{radius:nearest?9:6,color:nearest?"#a16207":style.color||"#ea580c",fillColor:nearest?"#facc15":style.fillColor||"#f97316",fillOpacity:.85,weight:2}).bindPopup(`${r.siteName||"Relave"}<br>${formatDistance(r.distanceKm)}`).addTo(layers.results); }); if(Number.isFinite(res.clusterRadiusKm ?? res.radiusKm)&&rels[0]) L.circle([queryLat,queryLon],{radius:(res.clusterRadiusKm ?? res.radiusKm)*1000,color:style.color||"#ea580c",dashArray:"4 8",fillColor:style.fillColor||"#f97316",fillOpacity:.12,weight:2}).addTo(layers.results); if(rels[0]) L.polyline([[queryLat,queryLon],[rels[0].coordinates[1],rels[0].coordinates[0]]],{color:"#38bdf8",dashArray:"4 6",weight:3}).addTo(layers.results); } else if(res.items[0]) { L.geoJSON(res.items[0].feature,{style:{color:style.color||"#7c2d12",fillColor:style.fillColor||"#fb923c",fillOpacity:style.fillOpacity??.25,weight:style.weight||2}}).addTo(layers.results); if(res.nearestPoint){ const c=res.nearestPoint.geometry.coordinates; L.polyline([[queryLat,queryLon],[c[1],c[0]]],{color:"#7c2d12",dashArray:"4 6"}).addTo(layers.results); } } }
