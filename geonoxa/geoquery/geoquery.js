...
function buildGeoNoxaMapExport(relavesResult, zonasResult) {
 const state=window.geoQueryState||{}; const folders=[{id:"query",name:"Punto consultado"},{id:"cluster",name:"Clúster de relaves"},{id:"relaves",name:"Relaves seleccionados"},{id:"zonas",name:"Zona saturada o latente"}];
 const features=[{id:"query-point",folderId:"query",type:"point",name:"Punto consultado",geometry:{type:"Point",coordinates:[state.lon,state.lat]},style:{color:"#111827",fillColor:"#facc15",fillOpacity:.95,weight:3},extendedData:{Latitud:state.lat,Longitud:state.lon,CRS:"WGS84 / EPSG:4326"},visible:true}];
 const rels=Array.isArray(relavesResult?.selectedRelaves)?relavesResult.selectedRelaves:(relavesResult?.items||[]);
 if(relavesResult?.status==="resolved"&&rels.length){ const radius=relavesResult.clusterRadiusKm??relavesResult.radiusKm; if(Number.isFinite(radius)){ const circle=turf.circle([state.lon,state.lat],radius,{steps:128,units:"kilometers"}); features.push({id:"relaves-cluster",folderId:"cluster",type:"polygon",name:"Círculo punteado del clúster de relaves",geometry:circle.geometry,style:{color:"#ea580c",opacity:1,weight:2,fill:false,fillOpacity:0},extendedData:{Radio:formatDistanceKm(radius),"Recurso dominante":relavesResult.dominantResource,Participación:relavesResult.dominantResourcePercentage,Cantidad:rels.length},visible:true}); }
  rels.slice(0,10).forEach((r,i)=>features.push({id:`relave-${i+1}`,folderId:"relaves",type:"point",name:`${i+1}. ${r.siteName||r.faena||"Relave"}`,geometry:{type:"Point",coordinates:r.coordinates},style:{color:"#ea580c",fillColor:"#f97316",fillOpacity:.85,weight:2,labelColor:"#ea580c"},extendedData:{Ranking:i+1,Faena:r.siteName||r.faena,Empresa:r.company||r.empresa,Recurso:r.resource||r.recurso,"Tipo de depósito":r.depositType,"Método constructivo":r.constructionMethod,Comuna:r.commune,Área:r.area,Distancia:formatDistanceKm(r.distanceKm)},visible:true})); }
 const z=zonasResult?.items?.[0]; if(zonasResult?.status==="resolved"&&z){ const style={color:"#7c2d12",fillColor:"#fb923c",fillOpacity:.25,weight:2}; features.push({id:"zona-saturada-latente",folderId:"zonas",type:z.feature?.geometry?.type?.toLowerCase(),name:z.name||"Zona saturada o latente",geometry:z.feature?.geometry,style,extendedData:{Nombre:z.name,Condición:z.condition,Contaminante:z.pollutant,Decreto:z.decree,Región:z.regionCode,Superficie:z.officialArea,Relación:zonasResult.relation,Distancia:formatDistance(zonasResult.distanceKm)},visible:true}); const label=turf.pointOnFeature(z.feature)?.geometry?.coordinates; if(label) features.push({id:"zona-label",folderId:"zonas",type:"label",name:z.name||"Zona",geometry:{type:"Point",coordinates:label},style:{labelColor:style.color,labelScale:1,scale:.1},visible:true}); if(zonasResult.relation!=="intersects"&&zonasResult.nearestPoint?.geometry?.coordinates){ const p=zonasResult.nearestPoint.geometry.coordinates; const line=[[state.lon,state.lat],p]; const mid=turf.midpoint(turf.point(line[0]),turf.point(line[1])).geometry.coordinates; features.push({id:"zona-nearest-line",folderId:"zonas",type:"line",name:"Distancia mínima al perímetro",geometry:{type:"LineString",coordinates:line},style:{color:style.color,weight:3,opacity:1,dashArray:"4 6"},extendedData:{Distancia:formatDistance(zonasResult.distanceKm)},visible:true}); features.push({id:"zona-contact",folderId:"zonas",type:"point",name:"Punto de contacto con perímetro",geometry:{type:"Point",coordinates:p},style:{color:"#111827",fillColor:"#ffffff",fillOpacity:1,weight:2,iconType:"contact"},visible:true}); features.push({id:"zona-distance-label",folderId:"zonas",type:"label",name:`Distancia mínima: ${formatDistance(zonasResult.distanceKm)}`,geometry:{type:"Point",coordinates:mid},style:{labelColor:style.color,labelScale:.9,scale:.1},visible:true}); } }
