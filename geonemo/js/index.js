...
function normalizeGeoQueryOriginState(raw, site = SITE_ID) {
  return geoQueryContext.normalizeState(raw, site);
}

function resolveViewportRestoreState(site = SITE_ID) {
  return geoQueryContext.resolve(site);
}

function captureGeoQueryOriginState({ site = SITE_ID, map, queryLat, queryLon, basemap, from }) {
  return geoQueryContext.capture({ site, map, queryLat, queryLon, basemap, from });
}

function persistOriginStateBeforeGeoQuery(originState) {
  return geoQueryContext.persist(originState);
}

function appendOriginStateToGeoQueryUrl(url, originState) {
  return geoQueryContext.appendToGeoQueryUrl(url, originState);
}

function restoreMapViewport(mapInstance, restoreState) {
  return geoQueryContext.restore(mapInstance, restoreState, {
    site: SITE_ID,
    applyBasemap: (basemap) => {
      if (typeof switchBaseMap === "function") switchBaseMap(basemap);
    },
    applyQueryPoint: (lat, lon) => {
      if (typeof setSelectedPoint === "function") {
        setSelectedPoint(lat, lon, "geoquery_restore");
      } else {
        selectedPoint = {
          lat,
          lon,
          source: "geoquery_restore",
          site: SITE_ID,
    ...
    onRestore: (state) => {
      viewportRestoreApplied = true;
      geoQueryRestoreState = state;
    }
  });
}
...
function installGeoQueryViewportRestoreHandlers() {
  geoQueryContext.installRestoreHandlers({
    site: SITE_ID,
    getMap: () => map,
    restore: (mapInstance, state) => restoreMapViewport(mapInstance, state)
  });
}
...
async function registrarConsultaGeoNemo({ lat, lon, zoom, basemap, origen }) {
...
async function openGeoQueryFromLatLng(lat, lon) {
  ...
  try {
    const consultaId = await registrarConsultaGeoNemo({ lat, lon, zoom, basemap, origen: originState.navigation.crossAccess ? "cross_access" : "directo" });
