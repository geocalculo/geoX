/* GeoQuery 2.0 GeoNEMO · versión de producción. */
...
function kmlDescription(result) {
  return `Grupo: ${result.nombre}; Categoría: ${result.categoria}; Posición: ${result.posicion}; Superficie: ${formatNumber(result.superficieHa, 2)} ha; Distancia al borde: ${formatNumber(result.distanciaBordeKm, 3)} km; Diámetro equivalente: ${formatNumber(result.diametroEquivalenteKm, 3)} km; Relación territorial: ${formatNumber(result.relacionDiametros, 3)} diámetros; Profundidad relativa: ${Number.isFinite(result.profundidadRelativa) ? formatNumber(result.profundidadRelativa, 3) : "No aplica"}; ICT: ${ictDisplay(result)}; Clasificación: ${ictClassification(result).label}`;
}
...
function exportKml() {
  ...
  anchor.download = "geonemo_geoquery_todos_los_grupos.kml";
  anchor.click();
