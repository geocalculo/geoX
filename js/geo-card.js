...
function getPoiFromUrl() {
  ...
  const sitio = (params.get('sitio') || 'GeoX').trim() || 'GeoX';
...
// Futuro GeoFactory: estas fuentes podrán parametrizarse desde /parametros/parametros_card.json,
// /parametros/reglas_topologicas.json y /parametros/listado_card.json cuando existan.
...
async function buildAnalysisData(poi) {
  ...
  return {
    poi: { ...poi, name: `POI seleccionado${poi.sitio ? ` · ${poi.sitio}` : ''}`, comuna: 'Sin datos disponibles', region: poi.region || 'Sin datos disponibles' },
    zonaSaturada: {
...
function renderHeader() {
  ...
  document.getElementById('header-card').innerHTML = `<div class="context-panel"><div class="header-grid">
    <div class="brand">
      <h1>GeoX | Geo-Card / GeoQuery</h1>
      <p>Análisis territorial del POI · ${escapeHtml(analysisData.poi.sitio || 'GeoX')}</p>
    </div>
    <div class="meta">
      <p>POI: ${analysisData.poi.lat}, ${analysisData.poi.lon}</p>
      <p>Región: ${escapeHtml(analysisData.poi.region || 'Sin datos disponibles')}</p>
...
function buildEcosystemUrl(baseUrl) { const { lat, lon, zoom, bbox, sitio, region } = analysisData.poi; const bboxText = Array.isArray(bbox) ? bbox.join(',') : ''; const params = new URLSearchParams({ lat, lon, zoom, sitio: sitio || '', region: region || '' }); if (bboxText) params.set('bbox', bboxText); return `${baseUrl}?${params.toString()}`; }
